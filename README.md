...
```bash
jicl-train --config configs/lite_llava_caption.yaml
```

または:

```bash
python -m jicl.train --config configs/lite_llava_caption.yaml
```

GPU学習用スクリプト:

```bash
bash scripts/train_gpu.sh configs/lite_llava_caption.yaml
```

複数GPUを使う場合:

```bash
NUM_PROCESSES=2 bash scripts/train_gpu.sh configs/lite_llava_caption.yaml
```
...
```bash
DTYPE=bf16 ./scripts/generate_gpu.sh \
  --checkpoint outputs/lite-captioner \
  --image path/to/image.jpg
```
